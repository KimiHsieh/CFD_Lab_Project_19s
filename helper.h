...
//
// ============== parameters to change when changing convention ==========
#define SHIFT_EWSN 5  // 5 (WS2)
#define MASK_FLUID 4  // 4 (WS2)
#define FLAGS_BC 0xf  // 0xf (WS2)
// ==============  end ===================================================

// define section for EWSN ((Flag[i][j] >> SHIFT_EWSN) & 0b1111)
#define N 0x1          // 0b0001 (1)
#define S 0x2          // 0b0010 (2)
#define W 0x4          // 0b0100 (4)
#define E 0x8          // 0b1000 (8)
#define NW 0x5         // 0b0101 (5)
#define SW 0x6         // 0b0110 (6)
#define NE 0x9         // 0b1001 (9)
#define SE 0xA         // 0b1010 (10)
#define SN 0x3         // 0b0011 (3)
#define EW 0xC         // 0b1100 (12)
#define EWSN_MASK 0xF  // 0b1111 (F)

// BC types (Flag[i][j] & FLAGS_BC)
#define NO_SLIP 0x2    // 0b00.0010 (2)
#define FREE_SLIP 0x4  // 0b00.0100 (4)
#define OUTFLOW 0x8    // 0b00.1000 (8)
#define INFLOW 0x10    // 0b01.0000 (16)
//

/* ----------------------------------------------------------------------- */
/*                             print matrices                              */
/* ----------------------------------------------------------------------- */
void print_imatrix(int **m, int nrl, int nrh, int ncl, int nch);
void print_matrix(double **m, int nrl, int nrh, int ncl, int nch);
void search_ivalue(int *m, int n, int value, int *is_value);
 ...
 * READ_STRING( szFile, szProblem );
 */
#define READ_DOUBLE(szFileName, VarName) \
  read_double(szFileName, #VarName, &(VarName))
 ...
 * READ_STRING( szFile, szProblem );
 */
#define READ_STRING(szFileName, VarName) \
  read_string(szFileName, #VarName, (VarName))
 ...
 */
void write_matrix(const char *szFileName, double **m, int nrl, int nrh, int ncl,
                  int nch, double xlength, double ylength, int fFirst);
...
/**
 * reads in a ASCII pgm-file and returns the colour information in a
 ...
 *  respective rows by switching to the compilation-minor-mode.
 *
...
#define DUMP_POSITION() \
  fprintf(DUMPOUT, "%s:%d Dumpposition \n", __FILE__, __LINE__)
 ...
 *                    RESET_TIMER()
 */
#define DUMP_DOUBLE(d) \
  fprintf(DUMPOUT, "%s:%d %s = %f\n", __FILE__, __LINE__, #d, d)
 ...
 *                    RESET_TIMER()
 */
#define DUMP_STRING(s) \
  fprintf(DUMPOUT, "%s:%d %s = %s\n", __FILE__, __LINE__, #s, s)
 ...
 */
#define DUMP_TIMER()                                        \
  fprintf(DUMPOUT, "%s:%d Timer: %f\n", __FILE__, __LINE__, \
          (float)(clock() - last_timer_reset) / (float)CLOCKS_PER_SEC)
...
#define DUMP_MATRIX_TO_FILE(m, nrl, nrh, ncl, nch, xlength, ylength)  \
  {                                                                   \
    ...
    write_matrix(szFileName, m, nrl, nrh, ncl, nch, xlength, ylength, \
                 nCount == 0);                                        \
    ++nCount;                                                         \
  }
