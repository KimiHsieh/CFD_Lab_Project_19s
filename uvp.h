 ...
 * @f$ G_{i,j} := v_{i,j} + \delta t \left( \frac{1}{Re} \left(
   \left[ \frac{\partial^2 v}{\partial x^2}\right]_{i,j} + \left[
 \frac{\partial^2 v}{\partial y^2} \right]_{i,j} \right) - \left[ \frac{\partial
 ...
 *
 * @f$ rs = \frac{1}{\delta t} \left(
 * \frac{F^{(n)}_{i,j}-F^{(n)}_{i-1,j}}{\delta x} +
 * \frac{G^{(n)}_{i,j}-G^{(n)}_{i,j-1}}{\delta y} \right)  @f$
 *
 */
 ...
 * by
 *
 * @f$ {\delta t} := \tau \, \min\left( \frac{Re}{2}\left(\frac{1}{{\delta x}^2}
 * + \frac{1}{{\delta y}^2}\right)^{-1},  \frac{{\delta
 * x}}{|u_{max}|},\frac{{\delta y}}{|v_{max}|} \right) @f$
 *
 */
 ...
 *
 * @f$ u_{i,j}^{(n+1)}  =  F_{i,j}^{(n)} - \frac{\delta t}{\delta x}
 * (p_{i+1,j}^{(n+1)} - p_{i,j}^{(n+1)}) @f$
 * @f$ v_{i,j}^{(n+1)}  =  G_{i,j}^{(n)} - \frac{\delta t}{\delta y}
 * (p_{i,j+1}^{(n+1)} - p_{i,j}^{(n+1)}) @f$
 *
