...
void print_imatrix(int **m, int nrl, int nrh, int ncl, int nch) {
  printf("Print matrix:\n\n");
  for (int j = nch; j >= ncl; --j) {
    printf("%d.\t", j);
    for (int i = nrl; i <= nrh; ++i) printf("%d ", m[i][j]);
    printf("\n");
  }
}
...
void print_matrix(double **m, int nrl, int nrh, int ncl, int nch) {
  ...
  for (int j = nch; j >= ncl; --j) {
    printf("%d.\t", j);
    for (int i = nrl; i <= nrh; ++i) printf("%.2f ", m[i][j]);
    printf("\n");
  }
}
...
char *find_string(const char *szFileName, const char *szVarName) {
  ...
  if (fh == 0) READ_ERROR("Could not open file", szVarName, szFileName, 0);
                  ...
                  int fFirst)             /* 0 == append, else overwrite*/
{
  ...
  for (int j = ncl; j <= nch; ++j)
    for (int i = nrl; i <= nrh; ++i) tmp[k++] = (float)m[i][j];
...
void read_matrix(const char *szFileName, /* filename */
  ...
  for (int j = ncl; j <= nch; ++j)
    for (int i = nrl; i <= nrh; ++i) m[i][j] = tmp[k++];
